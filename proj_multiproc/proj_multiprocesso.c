...
int main(int argc, char const *argv[]) {
    ...
    while(exit1 != 0){
        ...
        if(carattere == L'a'){
            ...
            //Dichiaro i due figli
            pid_t p1, p2;

            //Effettuo la fork del primo figlio
            p1 = fork();

            if (p1 < 0) {
                printf("Errore nel fork del primo figlio\n");
                return 1;
            } else if (p1 == 0) {
                ...
                // i in parole_uniche2 corrisponderà alla stessa posizione i di conteggio_parola_unica il numero di volte che appare nel file
                int conteggio_parola_unica[indice_parole_uniche];

                //Questo array conterrà tutte le parole che seguono la singola parola in parole_uniche2
                ...
                //In questo array saranno inserite le righe da stampare nel file .csv
                int dimensione = 50000;
                wchar_t** array = malloc(dimensione * sizeof(wchar_t*));

                //Prendo parola per parola nell'array parole_uniche2
                for(int i = 0; i < indice_parole_uniche; i++){   
                    int conteggio_parole = 0;
                    //Prendo parola per parola in tutto_il_file2
                    for (int j = 0; j < indice_tutto_file; j++){
                        ...
                        //Effettuo comparazione tra la parola in parole_uniche2 con tutte quelle in tutto_il_file2
                    ...
                    array[i] = stampa_tabella(parole_che_seguono, parole_uniche,conteggio_parole_che_seguono, conteggio_parola_unica, i, indice_parole_che_seguono);
                ...
                //Effettuo la fork del secondo figlio
                p2 = fork();

                if (p2 < 0) {
                    printf("Errore nella fork del secondo figlio\n");
                    return 1;
                } else if (p2 == 0) {
            ...
            //Calcolo il numero di righe nel .csv per generare successivamente una parola di inizio casuale, prendo carattere per carattere e verifico se il nuovo 
            //sia uguale al precedente e uguali a '\n', in caso affermativo vuol dire che c'è una linea vuota
            int numero_righe = 0;
            wchar_t c = L'\0';
            wchar_t prev_c = L'\n';
            while(c = fgetwc(fp), c != WEOF ){
                if(c == L'\n' && prev_c != L'\n')
                    numero_righe++;
                prev_c = c;
            }
            ...
            if (p3 < 0) {
                ...
                while(contatore_lunghezza_frase < lunghezza_frase_generata){
                    ...
                    while (fgetws(line, sizeof(line), fp)){
                        ...
                        if ( conteggio == parola_inizio_frase ||  (wcscmp(parola_da_cercare, token) == 0)){
                            ...
                            //Verifico che il file sia corretto sommando tutte le frequenze di una singola riga, poichè il programma entrava in errore con la verifica "checksum > 1"
                            //nonostante la somma fosse esattamente 1 ho inserito una soglia per il confronto, se quindi la somma - 1 = numero maggiore di 0.00001 va in errore poichè vuol dire che somma > 1 
                            double checksum = 0;
                            for(int i = 0; i < posiz; i++){
                                checksum += frequenze[i];
                                if((checksum-1.0f) > 0.0001f){
                                    wprintf(L"-->Errore nel file csv, la somma delle frequenze è irregolare (%f) alla riga \"%ls\"\n",checksum, parola_da_cercare);
                                    return 1;
                                }
                            }
                ...
                p4 = fork();
                if (p4 < 0) {
                    printf("Errore nel fork del secondo figlio\n");
                    return 1;
                } else if (p4 == 0) {
                    ...
                    //Apertura/creazione file in cui inserire la frase generata
