...
//Stampa la tabella
void stampa_tabella(FILE *puntatore, wchar_t **parole_che_seguono, wchar_t **parole_uniche,int *conteggio_parole_che_seguono, int *conteggio_parola_unica, int i, int indice_parole_che_seguono){
    
    //Inserisco la parola analizzata
    fwprintf(puntatore, L"%ls",parole_uniche[i]);
    ...
    for (int m=0; m < indice_parole_che_seguono; m++){
        ...
        if ((double)conteggio_parole_che_seguono[m]/(double)conteggio_parola_unica[i] == 1){
            fwprintf(puntatore, L",%ls,%d", parole_che_seguono[m], conteggio_parole_che_seguono[m]/conteggio_parola_unica[i]);

        }else if((double)conteggio_parole_che_seguono[m]/(double)conteggio_parola_unica[i] == 0.5){
        
            fwprintf(puntatore, L",%ls,%.1f", parole_che_seguono[m], (double)conteggio_parole_che_seguono[m]/(double)conteggio_parola_unica[i]);

        }else{
            fwprintf(puntatore, L",%ls,%.4f", parole_che_seguono[m], (double)conteggio_parole_che_seguono[m]/(double)conteggio_parola_unica[i]);
        }  
    }
...
FILE* apertura_file_analisi(int codice_file){
    ...
    while(file_trovato == 0) {
        ...
        estensione++; // Spostati al primo carattere dopo il punto
